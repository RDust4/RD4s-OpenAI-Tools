"""
This file is a script that allows the user to generate audio files from text files using OpenAI's Text-to-Speech API.
Each .txt file in the `audio` directory is processed, and the text is split into chunks that are each converted to audio.
The resulting audio chunks are then combined into a single audio file, which is saved as an .mp3 file with the same name as the original .txt file.
"""
